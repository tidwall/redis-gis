...
void parseSerializeAndCompare(const char *input){
    wktGeometry *wkt = NULL;
    wktErr err = wktParse(input, &wkt);
    assert(err == WKT_ERRNONE);
    char *text1 = wktText(wkt);
    assert(text1 != NULL);
    wktFree(wkt);
	err = wktParse(input, &wkt);
    assert(err == WKT_ERRNONE);
    char *text2 = wktText(wkt);
    assert(text2 != NULL);
    wktFree(wkt);
...
int test_WKT(){
	...
	testGeom(800, 2, randGeomPoint);
	testGeom(600, 2, randGeomLineString);
	testGeom(600, 2, randGeomMultiPoint);
	testGeom(400, 2, randGeomPolygon);
	testGeom(400, 2, randGeomMultiLineString);
	testGeom(200, 2, randGeomMultiPolygon);
	testGeom(100, 2, randGeomGeometryCollection);
	return 1;
}
